...
public class NonConvexMeshCollider : MonoBehaviour
{
    ...
    [Tooltip("将创建一个名为“CulrDes”的子游戏对象来存储生成的对撞机. \n\r更好的结构. \n\r碰撞器不会给 child game object. 因此，要在在冲突检查中检查 transform.parent.gameObject.")]
    public bool createChildGameObject = true;
    [Tooltip("需要更多的时间来计算，但会导致更多性能优化的对撞机 (less boxes).")]
    public bool avoidGapsInside = false;
    [Tooltip("确保所有的框对撞器都完全在网格内部生成。更昂贵的计算，但如果你需要避免假碰撞的对象非常接近另一个，例如环的链.")]
    public bool avoidExceedingMesh = false;
    [Tooltip("网格将被分割成每个轴的盒子数量 (x, y and z). \n\r更高的值导致更准确的对撞机，但另一方面，计算和碰撞检查更昂贵.")]
    public int boxesPerEdge = 20;
    [Tooltip("物理应用到生成的混合碰撞.")]
